...
>
{t("gallery.noImage")} </div>
);
if (count === 1)
return ( <div className="relative h-96 md:h-[600px]"> <Image src={images[0]} alt={pkg.title} fill className="object-cover" priority /> </div>
);
if (count === 2)
return ( <div className="grid grid-cols-1 md:grid-cols-2 gap-2 h-72 md:h-[500px]">
{images.map((src, i) => ( <div key={i} className="relative w-full h-full">
<Image src={src} alt={`${pkg.title}-${i}`} fill className="object-cover" /> </div>
))} </div>
);
return ( <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-2 h-96 md:h-[600px]"> <div className="relative md:col-span-2 md:row-span-2"> <Image src={images[0]} alt={pkg.title} fill className="object-cover" priority /> </div>
{images.slice(1, 4).map((src, i) => ( <div key={i} className="relative">
<Image src={src} alt={`${pkg.title}-${i + 1}`} fill className="object-cover" /> </div>
))} </div>
);
};

return ( <div className={mainBgClass}> <div className="max-w-7xl mx-auto p-4 md:p-8">
        ...
        <div className="flex flex-wrap items-center gap-4 mb-4">
          <div
            className={`text-sm font-bold py-1 px-3 rounded-full ${
              theme === "regular" ? "bg-blue-100 text-blue-800" : "bg-blue-900 text-blue-200"
            }`}
          >
          ...
          <div
            className={`text-sm font-bold py-1 px-3 rounded-full ${
              theme === "regular" ? "bg-gray-100 text-gray-800" : "bg-gray-700 text-gray-200"
            }`}
          >
            {pkg.category}
          </div>
            ...
            <div className="py-8">
              ...
              {activeTab === "Cost" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    <h4 className={`text-lg font-bold mb-4 ${textClass}`}>
                      {t("cost.included")}
                    </h4>
                    <ul className="space-y-2">
                      {pkg.cost?.included.map((item) => (
                        <li key={item} className="flex items-start gap-3">
                          <CheckIcon /> <span className={textMutedClass}>{item}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  <div>
                    ...
                    <ul className="space-y-2">
                      {pkg.cost?.excluded.map((item) => (
                        <li key={item} className="flex items-start gap-3">
                          <XIcon /> <span className={textMutedClass}>{item}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
          ...
          <div className={`border ${borderClass} rounded-xl shadow-lg p-6 sticky top-8`}>
            <div className={`flex justify-between divide-x ${borderClass} text-center mb-5`}>
              <div className="pr-4 flex-1">
                ...
                <p className={`text-sm ${textMutedClass}`}>/ {t("trip.adult")}</p>
              </div>
              <div className="pl-4 flex-1">
                <p className={`${textMutedClass} text-sm mt-[26px]`}>{t("trip.from")}</p>
                <p className={`text-2xl font-bold ${textClass}`}>
                  Rp{childPrice.toLocaleString("id-ID")}
                </p>
                <p className={`text-sm ${textMutedClass}`}>/ {t("trip.child")}</p>
              </div>
            </div>
