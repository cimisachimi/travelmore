...
interface PackageListItem {
  id: number;
  slug: string; // ✅ Added slug field to interface
  name: string; 
          ...
          <main className="w-full md:flex-1">
            ...
            {error && !loading && (
              <div className="text-center py-16 p-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl">
                <p className="text-red-600 dark:text-red-400 font-bold text-lg">{fetchErrorString}</p>
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-12">
                  {currentPackages.length > 0 ? (
                    currentPackages.map((pkg) => {
                      ...
                      return (
                        /* ✅ CHANGED: Now using pkg.slug for dynamic routing */
                        <Link key={pkg.id} href={`/packages/${pkg.slug}`} className="block group h-full">
                          <div className={`h-full flex flex-col rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border ${cardBgClass}`}>
                            ...
                            <div className="p-5 flex flex-col grow">
                              {pkg.category && <p className="text-xs font-bold text-primary uppercase tracking-wider mb-2">{pkg.category}</p>}
                              ...
                              <div className={`flex justify-between items-end mt-auto pt-4 border-t ${borderClass}`}>
                                <div>
                                  ...
                                  <p className="text-lg font-extrabold text-primary">{formatCurrency(startingPrice)}</p>
                                </div>
                    ...
                    <div className="lg:col-span-3 flex flex-col items-center justify-center py-16 text-center">
                      ...
                      <button onClick={() => { setSearchQuery(""); setMaxPrice(priceSliderMax); setSelectedCategories([]); }} className="mt-4 text-primary font-semibold hover:underline">Clear all filters</button>
                    </div>
                  )}
                </div>
                ...
                {totalPages > 1 && (
                  <div className="flex justify-center items-center gap-2 mt-8">
                    <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className={`p-2.5 rounded-lg border transition-all ${inactivePageClass} disabled:opacity-50 disabled:cursor-not-allowed`}><ChevronLeft size={18} /></button>
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                      <button key={page} onClick={() => handlePageChange(page)} className={`w-10 h-10 flex items-center justify-center rounded-lg border text-sm font-bold transition-all ${currentPage === page ? activePageClass : inactivePageClass}`}>{page}</button>
                    ))}
                    <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className={`p-2.5 rounded-lg border transition-all ${inactivePageClass} disabled:opacity-50 disabled:cursor-not-allowed`}><ChevronRight size={18} /></button>
                  </div>
                )}
              </>
            )}
