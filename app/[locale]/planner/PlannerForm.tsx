...
const FormInput = ({
  ...
  selectPlaceholder?: string;
    ...
    if (as === 'select') {
        return ( <div> <label className={labelClasses}>{label}</label> <select name={name} value={value as string} onChange={onChange} className={baseInputClasses}> <option value="" disabled>{selectPlaceholder}</option> {options.map(opt => { const val = typeof opt === 'string' ? opt : opt.value; const lab = typeof opt === 'string' ? opt : opt.label; return <option key={val} value={val}>{lab}</option>; })} </select> </div> );
    }
    if (as === 'textarea') {
