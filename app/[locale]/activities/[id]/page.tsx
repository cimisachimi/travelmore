...
export interface Activity {
  ...
  itinerary: string | null; // Now a text block (HTML/String)
  notes: string | null;     // New text field
  
  addons?: Addon[];
  images_url: { id: number; url: string; type: string }[]; 
  thumbnail_url: string | null;
  
  // Keep these if your backend still sends them, otherwise optional/remove
...
export default function ActivityDetailPage() {
  ...
  // ✅ Fallbacks using new data structure
    ...
    <div className={`min-h-screen ${mainBg} transition-colors duration-300`}>
      ...
      <div className="relative">
        ...
        <div className="hidden md:grid grid-cols-4 grid-rows-2 gap-2 h-[500px] max-w-7xl mx-auto pt-8 px-8">
           ...
           {/* ... Gallery images 2-5 remain the same ... */}
            ...
            <div className={`p-6 md:p-8 rounded-3xl border shadow-sm ${cardBg}`}>
              ...
              {activeTab === "Overview" && (
                <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  {/* ✅ Added Notes Section in Overview if available */}
                  {notesText && (
                    <div className={`mt-8 p-6 rounded-xl border-l-4 border-yellow-500 ${isDark ? "bg-yellow-900/10" : "bg-yellow-50"}`}>
                        <h4 className="flex items-center gap-2 font-bold text-lg mb-2 text-yellow-600 dark:text-yellow-500">
                            <AlertCircle size={20} /> Important Notes
                        </h4>
                        <p className={`whitespace-pre-wrap text-sm leading-relaxed ${textMuted}`}>
                            {notesText}
                        </p>
                    </div>
                  )}
              ...
              {activeTab === "Itinerary" && (
                <div className="space-y-6">
                  ...
                  {/* ✅ Updated: Render Text instead of Array Map */}
                  {itineraryText ? (
                    <div className={`prose ${isDark ? "prose-invert" : ""} max-w-none`}>
                        <div className={`whitespace-pre-wrap leading-relaxed ${textMuted}`}>
                            {itineraryText}
                        </div>
                    </div>
                  ) : (
              ...
              {activeTab === "Cost" && (
                <div className="grid md:grid-cols-2 gap-8">
                   ...
                   <div>
                     ...
                     <ul className="space-y-3">
                       {includesData.excluded?.length ? includesData.excluded.map((item, i) => (
                         <li key={i} className={`flex items-start gap-3 text-sm ${textMuted}`}>
              ...
              {activeTab === "Map" && (
                <div className="h-[400px] rounded-xl overflow-hidden bg-gray-200 relative">
                  {/* ✅ Updated: Use mapUrl (from google_map_url) */}
                  {mapUrl ? (
