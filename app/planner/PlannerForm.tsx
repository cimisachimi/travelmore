...
const FormInput: React.FC<any> = ({ label, name, value, onChange, placeholder, type = 'text', as, options, description, checked, onCheckboxChange }) => {
  if (as === 'select') {
    return (
      <div className="font-poppins">
        <label htmlFor={name} className="block text-sm font-medium text-gray-700 mb-1">{label}</label>
        <select id={name} name={name} value={value} onChange={onChange} className="w-full p-3 rounded-lg bg-gray-100 text-black border border-gray-300 focus:ring-2 focus:ring-primary focus:outline-none transition">
    ...
    return (
      <div className="font-poppins">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">
          {options.map((opt: any) => (
            <label key={opt.value || opt} className="flex items-center space-x-2 border border-gray-300 p-2 rounded-lg cursor-pointer transition-all hover:bg-gray-100">
              <input
                type="checkbox"
                name={name}
                value={opt.value || opt}
                checked={value.includes(opt.value || opt)}
    ...
    return (
      <div className="font-poppins">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          {options.map((opt: any) => (
            <label key={opt.value || opt} className="flex items-center space-x-2">
              <input
                type="radio"
                name={name}
                value={opt.value || opt}
  ...
  return (
    <div className="font-poppins">
      <label htmlFor={name} className="block text-sm font-medium text-gray-700 mb-1">{label}</label>
      {as === 'textarea' ? (
        <textarea id={name} name={name} value={value} onChange={onChange} placeholder={placeholder} rows={4} className="w-full p-3 rounded-lg bg-gray-100 text-black border border-gray-300 focus:ring-2 focus:ring-primary focus:outline-none transition" />
      ) : (
        <input id={name} type={type} name={name} value={value} onChange={onChange} placeholder={placeholder} className="w-full p-3 rounded-lg bg-gray-100 text-black border border-gray-300 focus:ring-2 focus:ring-primary focus:outline-none transition" />
      )}
    </div>
  );
};
...
export default function PlannerForm() {
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      setFormData((prev: any) => ({ ...prev, [name]: [...currentValues, value] }));
    } else {
      setFormData((prev: any) => ({ ...prev, [name]: currentValues.filter((item: any) => item !== value) }));
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div onClick={() => { setFormData((prev: any) => ({ ...prev, type: 'personal' })); handleNext(); }} className="p-6 text-center border-2 border-gray-300 rounded-lg shadow-md cursor-pointer hover:bg-blue-50 hover:border-primary transition duration-300 transform hover:-translate-y-1">
              ...
              <div onClick={() => { setFormData((prev: any) => ({ ...prev, type: 'company' })); handleNext(); }} className="p-6 text-center border-2 border-gray-300 rounded-lg shadow-md cursor-pointer hover:bg-blue-50 hover:border-primary transition duration-300 transform hover:-translate-y-1">
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div onClick={() => { setFormData((prev: any) => ({ ...prev, tripType: 'domestic' })); handleNext(); }} className="p-6 text-center border-2 border-gray-300 rounded-lg shadow-md cursor-pointer hover:bg-blue-50 hover:border-primary transition duration-300 transform hover:-translate-y-1">
                <h4 className="font-bold text-lg text-gray-900">Domestic</h4>
                <p className="text-sm text-gray-600">Dalam negeri</p>
              </div>
              <div onClick={() => { setFormData((prev: any) => ({ ...prev, tripType: 'foreign' })); handleNext(); }} className="p-6 text-center border-2 border-gray-300 rounded-lg shadow-md cursor-pointer hover:bg-blue-50 hover:border-primary transition duration-300 transform hover:-translate-y-1">
        ...
        {step === 5 && (
            <div className="space-y-4">
                ...
                <FormInput label="Anak-anak (0-12)" name="paxKids" value={formData.paxKids} onChange={handleChange} type="number" />
                <FormInput label="Remaja (13-17)" name="paxTeens" value={formData.paxTeens} onChange={handleChange} type="number" />
                <FormInput label="Dewasa (18-40)" name="paxAdults" value={formData.paxAdults} onChange={handleChange} type="number" />
                <FormInput label="Senior (40+)" name="paxSeniors" value={formData.paxSeniors} onChange={handleChange} type="number" />
                <button type="button" onClick={handleNext} className="w-full px-8 py-3 rounded-lg bg-primary text-gray-900 font-bold hover:brightness-90 transition">Lanjutkan</button>
            </div>
        )}
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-4">
                    {Object.keys(budgetPacks).map(pack => (
                        <div key={pack} onClick={() => setFormData((prev: any) => ({ ...prev, budgetPack: pack }))} className={`p-4 border-2 rounded-lg shadow-sm cursor-pointer transition duration-300 transform hover:-translate-y-1 ${formData.budgetPack === pack ? 'bg-secondary border-primary shadow-lg' : 'border-gray-300 hover:bg-gray-100'}`}>
                ...
                <div className="mt-6">
                    ...
                    <FormInput as="checkbox-group" label="Pilih add-on Anda" name="addons" options={addonOptions} value={formData.addons} onCheckboxChange={handleCheckboxChange} />
                </div>
                <button type="button" onClick={handleNext} className="w-full px-8 py-3 rounded-lg bg-primary text-gray-900 font-bold hover:brightness-90 transition">Lanjutkan</button>
            </div>
        )}
        ...
        {step === 8 && (
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    <FormInput as="checkbox-group" label="Pilih gaya perjalanan Anda" name="travelStyle" options={travelStyles} value={formData.travelStyle} onCheckboxChange={handleCheckboxChange} />
                </div>
                <div className="space-y-2">
                    <h4 className="font-semibold text-gray-700">Kepribadian Anda</h4>
                    <FormInput as="checkbox-group" label="Pilih kepribadian Anda" name="travelPersonality" options={travelPersonalities} value={formData.travelPersonality} onCheckboxChange={handleCheckboxChange} />
                </div>
                <button type="button" onClick={handleNext} className="w-full px-8 py-3 rounded-lg bg-primary text-gray-900 font-bold hover:brightness-90 transition">Lanjutkan</button>
            </div>
        )}
        ...
        {step === 10 && (
            <div className="space-y-4">
                ...
                <div className="flex items-center space-x-2">
                    <input type="checkbox" name="consent" checked={formData.consent} onChange={handleCheckboxChange} className="h-4 w-4 text-primary rounded" />
