    ...
    <div className={`${mainBgClass}`}>
      ...
      <header className={`py-12 ${headerBgClass} border-b ${theme === 'regular' ? 'border-gray-200' : 'border-gray-800'}`}>
        <div className="container mx-auto px-4 lg:px-8 text-center">
          <h1 className={`text-4xl md:text-5xl font-extrabold ${textClass}`}>Explore Our Tour Packages</h1>
          <p className={`mt-4 text-lg max-w-2xl mx-auto ${textMutedClass}`}>
            Find the perfect adventure in Yogyakarta, curated just for you. Adjust the filters to match your preferences.
          </p>
        </div>
      </header>
      ...
      <div className="container mx-auto px-4 lg:px-8 py-12">
        <div className="flex flex-col md:flex-row gap-8">
          {/* Filter Sidebar */}
          <aside className={`w-full md:w-1/4 md:block ${isFilterOpen ? 'block' : 'hidden'}`}>
            <div className={`${cardBgClass} p-6 rounded-lg shadow-md sticky top-24`}>
              <h3 className={`text-xl font-bold mb-4 ${textClass}`}>Filters</h3>
              <div className="mb-6">
                <label htmlFor="priceRange" className={`block font-semibold mb-2 ${textClass}`}>Price Range</label>
                <input id="priceRange" type="range" min={priceBounds.min} max={priceBounds.max} step="100000" value={maxPrice} onChange={(e) => setMaxPrice(Number(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700" />
                <div className={`mt-2 ${textMutedClass}`}>Up to: <strong>{formatCurrency(maxPrice)}</strong></div>
              </div>
                ...
                <div className="space-y-2">
                  {allCategories.map((category) => (
                    <label key={category} className="flex items-center">
                      <input type="checkbox" name={category} checked={selectedCategories.includes(category)} onChange={handleCategoryChange} className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-600" />
          ...
          <main className="w-full md:w-3/4">
            ...
            <div className="md:hidden mb-4">
              ...
              >
                <FilterIcon /> {isFilterOpen ? 'Close Filters' : 'Show Filters'}
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {filteredPackages.length > 0 ? (
                  ...
                  <Link key={pkg.id} href={`/packages/${pkg.id}`}>
                    <div className={`${cardBgClass} rounded-lg shadow-lg overflow-hidden flex flex-col group transition hover:shadow-2xl hover:-translate-y-1 h-full`}>
                      <div className="relative h-56 w-full overflow-hidden">
                        <Image src={pkg.images && pkg.images.length > 0 ? pkg.images[0] : '/placeholder.jpg'} alt={pkg.title} layout="fill" objectFit="cover" className="transition-transform duration-500 group-hover:scale-105" />
                        {/* [NEW] Duration Badge */}
                      ...
                      <div className="p-6 flex flex-col flex-grow">
                        {/* [NEW] Category Tag */}
                        <p className={`text-sm font-semibold text-blue-500 mb-2`}>{pkg.category}</p>
                        <h2 className={`text-xl font-bold mb-2 ${textClass}`}>{pkg.title.split(': ')[1]}</h2>
                        <p className={`mb-4 flex-grow ${textMutedClass} text-sm`}>{pkg.description}</p>
                        {/* [REDESIGNED] Price and CTA */}
                        <div className={`flex justify-between items-center mt-auto pt-4 border-t ${theme === 'regular' ? 'border-gray-100' : 'border-gray-700'}`}>
                          <p className={`text-lg font-bold text-blue-600 dark:text-blue-400`}>{formatCurrency(theme === 'regular' ? pkg.regularPrice : pkg.exclusivePrice)}</p>
                          <span className={`text-sm font-semibold ${textClass} group-hover:text-blue-500 transition`}>View Details &rarr;</span>
                        </div>
                      </div>
                    </div>
                  </Link>
                ...
                <div className="lg:col-span-2 text-center py-16">
                  <p className="text-gray-500 text-xl">No packages match your filters.</p>
                </div>
              )}
            </div>
          </main>
