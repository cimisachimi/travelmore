...
"use client"; // Diperlukan untuk menggunakan hook tema

import React from 'react';
import Link from 'next/link';
import Image from 'next/image';
import { useTheme } from '@/components/ThemeProvider'; // Impor useTheme
  ...
  return (
    <main className="bg-background transition-colors duration-500">
      ...
      <section className="text-center py-16 px-4">
        <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-3">Layanan Kami</h1>
            ...
            <div className="flex flex-col md:flex-row items-center">
              ...
              <div className="md:w-1/2 p-8 lg:p-12">
                <h2 className="text-3xl font-bold mb-4 text-foreground">
                  {isExclusive ? "Exclusive Trip Planner" : "Konsultasi Trip Planner"}
                </h2>
                <p className="text-foreground/80 mb-6">
                  {isExclusive
                    ? "Nikmati layanan premium dengan itinerary yang dirancang khusus oleh desainer perjalanan senior kami untuk pengalaman yang tak tertandingi."
                    : "Biarkan ahli lokal kami merancang itinerary yang dipersonalisasi berdasarkan anggaran, minat, dan gaya perjalanan Anda."}
                </p>
                <ul className="space-y-2 mb-8">
                  <li className="flex items-start"><CheckIcon /><span>{isExclusive ? "Revisi itinerary tanpa batas" : "Itinerary yang dipersonalisasi"}</span></li>
                  <li className="flex items-start"><CheckIcon /><span>{isExclusive ? "Pemesanan premium end-to-end" : "Rekomendasi akomodasi & aktivitas"}</span></li>
                  <li className="flex items-start"><CheckIcon /><span>{isExclusive ? "Dukungan concierge 24/7" : "Optimalisasi anggaran perjalanan"}</span></li>
                </ul>
